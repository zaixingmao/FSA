...
class ExpressionNtuple : private boost::noncopyable {
  public:
    ...
    void fill(const T& element, std::vector<double> pdfWeights, std::vector<int> pdfIDs, int idx = -1, bool do_commit=true);
...
template<class T> void ExpressionNtuple<T>::fill(const T& element,
  ...
  for (size_t i = 0; i < columns_.size(); ++i) {
    // Compute the function and load the value into the column.
    if(columnNames_[i].find("pdfWeights") != std::string::npos) columns_[i].setValue(pdfWeights);
