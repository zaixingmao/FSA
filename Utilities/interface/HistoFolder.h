...
class HistoFolder {
  ...
  void fill(const T& object, double weight, int idx, std::vector<double> pdfWeights, std::vector<int> pdfIDs);
...
// Recursive filling of histograms with additional 
template<typename T>
void HistoFolder<T>::fill(const T& object, double weight, int idx, std::vector<double> pdfWeights, std::vector<int> pdfIDs) {
  // Check if we are selecting and if it passes
  if (!selector_.get() || (*selector_)(object)) {
    // Fill this directories histos
    for (size_t iHisto = 0; iHisto < histos_.size(); ++iHisto) {
    ...
    if (ntuple_.get())
      ...
      ntuple_->fill(object, pdfWeights, pdfIDs, idx);                                                                                                               
